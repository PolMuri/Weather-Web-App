...
function updateWeatherImage(data) {
    ...
    // Ens injecta la imatge aquí de l'HTML, que és weather.img per el seu ID
    weatherImg.src = src;
}
...
function backImage(data) {
    ...
    // Treiem les dades per saber el temps general que fa sense grans concrecions i ho guardem a una variable (backUrl)
    ...
    if (backUrl == 'Thunderstorm') {
        ...
        let rainElement;
        // Creem una variable anomenada "comptador" i es defineix com 10.
        // Aquesta variable es utilitzda per determinar quantes gotes de pluja es crearan.
        let comptador = 10;
        // Creem una variable anomenada "espaiament" i es defineix com la amplada de la 
        ...
        // Creem un bucle "for" que itera fins a quan i és més petit que el comptador.
        for (let i = 0; i < comptador; i++) {
            ...
            // Establim la posició de la gota de pluja en l'eix X utilitzant la variable 
            // "espaiament" per calcular la posició correcta.
            rainElement.style.left = (i * espaiament) + 'px';
            // Establim la durada de l'animació de la gota de pluja utilitzant la 
            ...
            // Per canviar la durada de l'animació, si la volem fer més ràpida hem de baixar els valors de
            // animationDuration i pujar els de animationDelay
            rainElement.style.animationDelay = Math.random() * 4 + 's';
            // Afegim l'element "rain" al document mitjançant el mètode "appendChild".
        ...
        function createSnow() {
            // Creem un element DIV utilitzant el mètode "createElement" de l'objecte "document" i passant com a paràmetre "DIV".
            ...
            // Utilitzem l'style per definir la duració d'animació entre 5s i 10s i un delay entre 0s i 5s.
        ...
        // Hi hauria d'haver posat un else
...
function selectIdiom() {
    ...
    switch (idioma) {
        case 'Català':
            idiomaFetch = 'ca';
            // Agafem l'element amb l'id searchbox (que és el buscador), accedeiim a .placeholder 
...
function getLocationSuccess(pos) {
    ...
    // Per consola es mostren les dades també així per curiositat les podem saber
    console.log('Your current position is:');
    console.log(`Latitude : ${crd.latitude}`);
    console.log(`Longitude: ${crd.longitude}`);
    console.log(`More or less ${crd.accuracy} meters.`);

    // La funció getLocationSuccess crida aquesta funció GetWeatherByCoordsFive amb la latitud i longitud obtingudes per obtenir el temps
...
// Obtenim les dades meteorològiques per a les coordenades proporcionades per l'objecte pos que es passa com a paràmetre
...
async function GetWeatherByCoords(pos) {
    try {
        ...
        // Obtenim el nom de la ciutat utilitzant les coordenades a través del Reverse Geocoding
        ...
        // Assegurem que la crida a l'API va bé i que la resposta conté el nom de la ciutat
        if (reverseGeoData && reverseGeoData.length > 0 && reverseGeoData[0].name) {
            const cityName = reverseGeoData[0].name;

            // Obtenim les dades meteorològiques utilitzant les coordenades
            ...
            // Mostrem les dades a la interfície d'usuari utilitzant l'html 
            ...
            // Basat en la temperatura, posem una imatge o una altra
            // i el mateix amb el temps que faci i amb això li posarem també una imatge de fons o una altra
...
// Comprovem si s'ha obtingut alguna dada vàlida de geolocalització i retorna un objecte amb les coordenades de latitud i longitud. Si no es troben dades vàlides, retorna null.*/
async function getCoordinatesByCityName(cityName) {
    // Realitzem una crida a l'API de geolocalització utilitzant el nom de la ciutat proporcionat
    ...
    // Comprovem si s'ha obtingut alguna dada vàlida de geolocalització
...
async function getWeatherByName(cityName) {
    try {
        ...
        if (coordinates) {
            ...
            // Ara fem servir aquestes coordenades per fer la crida a la nova API One Call 3.0
            ...
            // Substituim les dades de l'html per les obtingudes per les de la API 3.0 (la nova versió, abans anàvem amb la 2.5)
            city.innerHTML = cityName;
            date.innerHTML = (new Date()).toLocaleDateString();
            temp.innerHTML = `${(responseData.current.temp.toFixed(1))} ºC`;
            weather.innerHTML = responseData.current.weather[0].description;
...
async function getWeatherByLocationFive(cityName) {
    try {
        // Obtenim les coordenades de la ciutat
        const coordinates = await getCoordinatesByCityName(cityName);

        if (coordinates) {
            const { lat, lon } = coordinates;
            // Fem la crida a l'API One Call 3.0 amb les coordenades obtingudes
...
// La variable d no s'utilitza en aquesta funció. La variable weekday 
